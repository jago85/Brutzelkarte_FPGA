...
#define GPIO_IO_BASE    (0x0000)
#define GPIO_DEBUG_BASE (0x0010)

#ifdef DEBUG
#define WRITE_DEBUG(x) MICO_GPIO_WRITE_DATA_BYTE0(GPIO_DEBUG_BASE, x);
#else
...
unsigned char ReadBit(void)
{
    ...
    // Read the data bit
    MICO_GPIO_READ_DATA_BYTE0(GPIO_IO_BASE, res);
...
void WriteBit(unsigned char b)
{
    ...
    if (b == 0)
    {
        // Write "1" to drive low
        MICO_GPIO_WRITE_DATA_BYTE0(GPIO_IO_BASE, 1);
    }
    ...
    // Write "0" to tristate the output
    MICO_GPIO_WRITE_DATA_BYTE0(GPIO_IO_BASE, 0);
}
...
int main(void)
{
    ...
    MICO_GPIO_READ_DATA_BYTE0(GPIO_IO_BASE, isPal);
    isPal = (isPal & 0x4) ? 1 : 0;
